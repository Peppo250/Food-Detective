...
Changes in this version:
  - Raw OCR / parse output shown in a copyable debug text box
  - Results stream in as SSE events arrive (80ms poll)
  - Timeout increased to 90s; status shows elapsed time
  - Camera capture via OpenCV if available
"""
...
STATUS_COLORS = {
    ...
    "caution": (AMBER_LIGHT,  AMBER_DARK,  "Caution"),
    "avoid":   (RED_LIGHT,    RED_DARK,    "Avoid"),
    "toxic":   (TOXIC_BG,     TOXIC_FG,    "Not Food!"),
    "unknown": (GRAY_LIGHT,   GRAY_DARK,   "Unknown"),
}

STATUS_ICON = {
    "safe": "✅", "caution": "⚠️", "avoid": "🚫",
    "toxic": "☠️", "unknown": "❓",
}
...
class FoodDetectiveApp:
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="🔬 Food Detective!", bg=GREEN_LIGHT,
                 fg=GREEN_DARK, font=("Helvetica Neue", 24, "bold")).pack()
        tk.Label(hdr, text="Take a photo of any food label — I'll check every ingredient!",
                 bg=GREEN_LIGHT, fg=GREEN_MID, font=("Helvetica Neue", 11)).pack()

        # Notebook — two tabs: Scanner | Debug
    ...
    def _build_scan_tab(self, parent):
        ...
        left.pack(side="left", fill="y", padx=(8, 10), pady=8)
    ...
    def _build_upload_panel(self, parent):
        tk.Label(parent, text="Step 1: Choose a photo", bg=BG,
                 fg=GRAY_DARK, font=("Helvetica Neue", 12, "bold")).pack(anchor="w")

        # Image drop zone
        self._drop_frame = tk.Frame(parent, bg=PURPLE_LIGHT,
                                    highlightthickness=2, highlightbackground="#AFA9EC",
        ...
        tk.Button(bf, text="📁  Pick from Files", command=self._pick_file,
                  bg=PURPLE_LIGHT, fg=PURPLE_DARK, font=("Helvetica Neue", 10, "bold"),
                  relief="flat", padx=8, pady=5, cursor="hand2").pack(fill="x", pady=(0, 5))

        try:
            import cv2
            tk.Button(bf, text="📷  Take a Photo", command=self._capture_camera,
                      bg=PURPLE_LIGHT, fg=PURPLE_DARK, font=("Helvetica Neue", 10, "bold"),
                      relief="flat", padx=8, pady=5, cursor="hand2").pack(fill="x", pady=(0, 5))
        except ImportError:
            pass

        self._scan_btn = tk.Button(parent, text="🔍  Scan Ingredients!",
        ...
        self._status_label = tk.Label(parent, text="", bg=BG, fg=GRAY_DARK,
                                      font=("Helvetica Neue", 9),
                                      wraplength=250, justify="center")
        self._status_label.pack(pady=(6, 0))

    def _build_results_panel(self, parent):
        tk.Label(parent, text="Step 2: Results", bg=BG,
                 fg=GRAY_DARK, font=("Helvetica Neue", 12, "bold")).pack(anchor="w")

        # Score banner
        self._score_frame = tk.Frame(parent, bg=GRAY_LIGHT, pady=10, padx=12)
        self._score_frame.pack(fill="x", pady=(4, 8))
        self._score_label = tk.Label(self._score_frame,
                                     text="Scan a food label to see results!",
    ...
    def _make_counter(self, parent, num, label, bg, fg):
        ...
        n = tk.Label(f, text=num, bg=bg, fg=fg, font=("Helvetica Neue", 20, "bold"))
        n.pack()
        tk.Label(f, text=label, bg=bg, fg=fg, font=("Helvetica Neue", 9, "bold")).pack()
        return n
    ...
    def _build_debug_tab(self, parent):
        tk.Label(parent,
                 ...
                 bg=BG, fg=GRAY_DARK, font=("Helvetica Neue", 10),
                 wraplength=800, justify="left").pack(anchor="w", padx=10, pady=(8, 4))

        btn_row = tk.Frame(parent, bg=BG)
        btn_row.pack(fill="x", padx=10, pady=(0, 4))
        tk.Button(btn_row, text="📋  Copy All", command=self._copy_debug,
                  bg=PURPLE_LIGHT, fg=PURPLE_DARK, font=("Helvetica Neue", 10, "bold"),
                  relief="flat", padx=8, pady=4, cursor="hand2").pack(side="left")
        tk.Button(btn_row, text="🗑  Clear", command=self._clear_debug,
                  bg=GRAY_LIGHT, fg=GRAY_DARK, font=("Helvetica Neue", 10, "bold"),
                  relief="flat", padx=8, pady=4, cursor="hand2").pack(side="left", padx=(6, 0))

        self._debug_box = scrolledtext.ScrolledText(
            parent, font=("Courier New", 10), bg="#1E1E1E", fg="#D4D4D4",
    ...
    def _debug_log(self, text: str):
        """Append a line to the debug box (safe to call from any thread via queue)."""
    ...
    def _poll_results(self):
        ...
        try:
            while True:
                ...
                if event == "_debug":
                    # Write to debug box on main thread
                    self._debug_box.config(state="normal")
                    ...
                    score_str = f"  [{'cache' if data.get('from_cache') else 'api'}]"
                    self._debug_log(f"  {status:8s}  {data.get('name','')}{score_str}")
    ...
    def _finish_scan(self, data: dict):
        ...
        for w in self._score_frame.winfo_children():
            w.destroy()
        self._score_frame.config(bg=bg, highlightthickness=2, highlightbackground=border)
    ...
    def _add_ingredient_card(self, data: dict):
        ...
        card = tk.Frame(self._results_frame, bg=bg, padx=10, pady=7)
        card.pack(fill="x", padx=4, pady=2)

        top = tk.Frame(card, bg=bg)
        top.pack(fill="x")

        tk.Label(top, text=f"{icon} {badge}", bg=bg, fg=fg,
        ...
        if explain:
            tk.Label(card, text=explain, bg=bg, fg=fg,
                     font=("Helvetica Neue", 9),
                     wraplength=500, justify="left", anchor="w").pack(fill="x", pady=(3, 0))
